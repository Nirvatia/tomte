<script lang="ts">
  import { X, Download, Eye, Edit3, Trash2, Plus, Archive } from '@lucide/svelte';
  import { isExtractorOpen, attachedFiles } from '../../stores';
  import type { ExtractedFile } from '../../utils/extractor';
  import { extractFilesFromMarkdown, convertToAttachedFile, downloadFile, createZipBlob } from '../../utils/extractor';
  ...
  function close() { isExtractorOpen.set(false); }
  ...
  function handlePreview(file: ExtractedFile) { previewFile = file; }
  function closePreview() { previewFile = null; }
</script>
...
{#if $isExtractorOpen}
  ...
  >
    <div class="bg-surface rounded-2xl shadow-2xl max-w-7xl w-full h-[90vh] flex flex-col" role="presentation" onclick={(e) => e.stopPropagation()}>
      <!-- Заголовок -->
      <div class="flex items-center justify-between p-6 border-b border-slate-100">
        <div>
          ...
          <p class="text-sm text-ink-tertiary mt-1">Извлечение файлов из markdown-ответов LLM</p>
        </div>
        <button type="button" onclick={close} class="p-2 rounded-lg hover:bg-surface-tertiary text-ink-secondary hover:text-ink transition-colors" aria-label="Закрыть">
          <X size={20} />
        </button>
      </div>
      ...
      <div class="flex-1 flex overflow-hidden">
        ...
        <div class="flex-1 flex flex-col border-r border-slate-100">
          <div class="p-4 border-b border-slate-100 bg-surface-secondary">
            <label for="markdown-input" class="block text-sm font-medium text-ink mb-2">Вставьте ответ LLM (Markdown):</label>
          </div>
        ...
        <div class="flex-1 flex flex-col">
          <div class="p-4 border-b border-slate-100 bg-surface-secondary">
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-xs text-ink-tertiary mt-0.5">{extractedFiles.length} файл(ов)</p>
              </div>
              {#if extractedFiles.length > 0}
                <button type="button" onclick={handleAddToAttachments} class="px-3 py-1.5 text-xs font-medium rounded-lg bg-brand-500 text-white hover:bg-brand-600 transition-colors">
                  Добавить в вложения
                </button>
              {/if}
            </div>
          ...
          <div class="flex-1 overflow-y-auto p-4">
            {#if extractedFiles.length === 0}
              <div class="text-center py-20 text-ink-tertiary">
                ...
                <p class="text-sm mt-2">Убедитесь, что текст содержит блоки кода в формате ```</p>
              </div>
            {:else}
              <div class="space-y-2">
                {#each extractedFiles as file, index (file.id)}
                  <div class="group flex items-center gap-3 p-3 bg-surface-secondary rounded-xl border border-slate-100 hover:border-purple-200 hover:shadow-soft transition-all">
                    <div class="w-10 h-10 rounded-lg bg-purple-50 flex items-center justify-center flex-shrink-0">
                      <span class="text-purple-600 text-xs font-bold">{file.lang.toUpperCase()}</span>
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      {:else}
                        ...
                        <p class="text-xs text-ink-tertiary mt-0.5">{file.code.length} символов</p>
                      {/if}
                    </div>

                    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {#if editingIndex === index}
                        <button type="button" onclick={saveRename} class="p-1.5 rounded-lg hover:bg-emerald-50 text-emerald-600 transition-colors" title="Сохранить"><Plus size={16} /></button>
                        <button type="button" onclick={cancelRename} class="p-1.5 rounded-lg hover:bg-surface-tertiary text-ink-secondary transition-colors" title="Отмена"><X size={16} /></button>
                      {:else}
                        <button type="button" onclick={() => handlePreview(file)} class="p-1.5 rounded-lg hover:bg-surface-tertiary text-ink-secondary hover:text-ink transition-colors" title="Просмотр"><Eye size={16} /></button>
                        <button type="button" onclick={() => handleRename(index)} class="p-1.5 rounded-lg hover:bg-surface-tertiary text-ink-secondary hover:text-ink transition-colors" title="Переименовать"><Edit3 size={16} /></button>
                        <button type="button" onclick={() => handleDownload(index)} class="p-1.5 rounded-lg hover:bg-surface-tertiary text-ink-secondary hover:text-ink transition-colors" title="Скачать"><Download size={16} /></button>
                        <button type="button" onclick={() => handleRemove(index)} class="p-1.5 rounded-lg hover:bg-red-50 text-ink-secondary hover:text-red-500 transition-colors" title="Удалить"><Trash2 size={16} /></button>
                      {/if}
                    </div>
                  </div>
                {/each}
          ...
          {#if extractedFiles.length > 0}
            <div class="p-4 border-t border-slate-100 bg-surface-secondary">
              <button type="button" onclick={handleDownloadAll} class="w-full px-4 py-2.5 bg-surface-tertiary text-ink rounded-lg font-medium hover:bg-slate-200 transition-colors flex items-center justify-center gap-2">
                <Archive size={16} />
...
{#if previewFile}
  ...
  >
    <div class="bg-surface rounded-2xl shadow-2xl max-w-4xl w-full max-h-[85vh] flex flex-col" role="presentation" onclick={(e) => e.stopPropagation()}>
      <div class="flex items-center justify-between p-6 border-b border-slate-100">
        <div>
          <h3 class="text-lg font-bold text-ink">{previewFile.name}</h3>
          <p class="text-sm text-ink-tertiary mt-0.5">{previewFile.lang.toUpperCase()} · {previewFile.code.length} символов</p>
        </div>
        <button type="button" onclick={closePreview} class="p-2 rounded-lg hover:bg-surface-tertiary text-ink-secondary hover:text-ink transition-colors" aria-label="Закрыть предпросмотр">
          <X size={20} />
        </button>
      </div>
